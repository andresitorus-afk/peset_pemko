<template>
  <aside :class="[
    ...
    <button
      @click="handleLogout"
      class="flex items-center gap-3 w-full px-4 py-3 text-sm font-medium border-t border-slate-200 dark:border-gray-800 text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-gray-800 transition-colors"
    >
      ...
      <span v-if="!collapsed" class="transition-opacity duration-200">Logout</span>
    </button>
...
const menuItems = [
  ...
  { to: '/admin/laporan', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 17v-2m3 2v-4m3 4v-6m2 10H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.707.293V19a2 2 0 01-2 2z"/></svg>', label: 'Laporan', notifs: 0 },
  { to: '/admin/riwayat-aset', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z"/></svg>', label: 'Audit Trail', notifs: 0 },
  { to: '/admin/live-chat', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 14v-2a8 8 0 0 1 16 0v2M3 14a1 1 0 0 1 1-1h1a1 1 0 0 1 1 1v4a1 1 0 0 1-1 1H4a1 1 0 0 1-1-1zm15-1h1a1 1 0 0 1 1 1v4a1 1 0 0 1-1 1h-1a1 1 0 0 1-1-1v-4a1 1 0 0 1 1-1z"/></svg>', label: 'Live Chat', notifs: 0, liveChat: true },
  { to: '/admin/users', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4.354a4 4 0 110 5.292M15 21H3v-1a6 6 0 0112 0v1zm0 0h6v-1a6 6 0 00-9-5.197m13.5-9a2.5 2.5 0 11-5 0 2.5 2.5 0 015 0z"/></svg>', label: 'Users', notifs: 0, superAdminOnly: true },
  { to: '/admin/roles', icon: '<svg fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z"/></svg>', label: 'Roles', notifs: 0, superAdminOnly: true },
]

const visibleMenuItems = computed(() => menuItems.filter(i => !i.superAdminOnly || isSuperAdmin))

function badgeFor(item: { to: string; notifs: number; liveChat?: boolean }) {
  return item.liveChat ? unreadTotal.value : item.notifs
}
